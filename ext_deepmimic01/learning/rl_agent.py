...
class RLAgent(ABC):
    ...
    def _record_goal(self):
        g = self.world.env.record_ground(self.id) #self.world.env.record_goal(self.id)
        g = np.reshape(g, (32, 32))
        g = g[::4, ::4].flatten()
        return g

    def _record_ground(self):
        gr = self.world.env.record_ground(self.id)
        return gr
