...
void cSceneImitate::UpdateKinChar(double timestep)
{
	// printf("%s\n", "++++++++++++");
	const auto& kin_char = GetKinChar();
	// std::cout << kin_char->GetPose0().segment(0,3) << std::endl;
...
void cSceneImitate::ResetCharacters()
{
	...
	// tVector direction;
	// direction << 0, 1, 0, 0;
	// AddForce(0, 1, 1000, direction, 0.1);
}
...
void cSceneImitate::AddForce(int agent_id, int part_idx, double force, tVector direction, double time_step)
{
	// printf("%s\n", "Adding Force.");
	const std::shared_ptr<cSimCharacter>& curr_char = GetCharacter(agent_id);
	const auto& part = curr_char->GetBodyPart(part_idx);
	// ApplyRandForce(100, 110, 0.1, 0.011, part.get());
	cSimObj* obj;
	obj = part.get();
	assert(obj != nullptr);
	...
	perturb.mPerturb[0] = direction[0];
	perturb.mPerturb[1] = direction[1];
	perturb.mPerturb[2] = direction[2];
	perturb.mPerturb = force * perturb.mPerturb.normalized();
	perturb.mDuration = time_step;
