...
void cDrawRLScene::RecordGround(int agent_id, Eigen::VectorXd& out_state) const
{
	GetRLScene()->RecordGround(agent_id, out_state);
}
...
void cDrawRLScene::SetGoal(int agent_id, const Eigen::VectorXd& goal)
{
	GetRLScene()->SetGoal(agent_id, goal);
}

void cDrawRLScene::AddForce(int agent_id, int part_idx, double force, tVector direction, double time_step)
{
	GetRLScene()->AddForce(agent_id, part_idx, force, direction, time_step);
}
