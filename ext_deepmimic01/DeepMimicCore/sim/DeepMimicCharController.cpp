...
const int gDefaultNumGroundSamples = 1024;
...
cDeepMimicCharController::cDeepMimicCharController() : cCharController()
{
	...
	mPrevActionCOM.setZero();
        mViewDist = 1;
	mNumGroundSamples = gDefaultNumGroundSamples;
        mGroundSamples.resize(gDefaultNumGroundSamples);
...
tMatrix cDeepMimicCharController::BuildGroundSampleTrans() const
{
	double ground_h = 0;
	if (mGround != nullptr)
	{
		tVector root_pos = mChar->GetRootPos();
		ground_h = mGround->SampleHeight(root_pos);
...
void cDeepMimicCharController::ParseGround()
{
...
void  cDeepMimicCharController::RecordGround(Eigen::VectorXd& out_state)
{
	Eigen::VectorXd ground;
	//out_state = BuildGroundSampleTrans();
	ParseGround();
	out_state = mGroundSamples;
}
...
void cDeepMimicCharController::SampleGround(Eigen::VectorXd& out_samples) const
{
	for (int i = 0; i < mNumGroundSamples; ++i)
	{
...
tVector cDeepMimicCharController::CalcGroundSamplePos(int s) const
{
	//------------------------------------------------
	double max_dist = 1.5;//GetMaxViewDist();
	double min_dist = -0.5;//GetMinViewDist();
	...
	//------------------------------------------------
	tVector size   = tVector(w, 0, w, 0);//CalcGroundSampleSize();

	const int num_samples = mNumGroundSamples;//GetNumGroundSamples();
	int sample_res = 32;//GetGroundSampleRes();
	...
	sample_pos[3] = 0;
	return sample_pos;
	/*double view_dist = this->mViewDist;// GetViewDist();
	const double gViewMin = -0.5;
	const int num_samples = mNumGroundSamples;
...
void cDeepMimicCharController::BuildPoliStateGround(Eigen::VectorXd& out_ground) const
{
