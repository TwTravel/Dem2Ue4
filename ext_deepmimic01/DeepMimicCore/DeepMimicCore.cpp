...
std::vector<double> cDeepMimicCore::RecordGround(int agent_id) const
{
	const auto& rl_scene = GetRLScene();
	if (rl_scene != nullptr)
	{
		Eigen::VectorXd ground;
		rl_scene->RecordGround(agent_id, ground);

		std::vector<double> out_ground;
		ConvertVector(ground, out_ground);
		return out_ground;
	}
	return std::vector<double>(0);
...
void cDeepMimicCore::SetGoal(int agent_id, const std::vector<double>& goal)
{
	const auto& rl_scene = GetRLScene();
	if (rl_scene != nullptr)
	{
		Eigen::VectorXd in_goal;
		ConvertVector(goal, in_goal);
		rl_scene->SetGoal(agent_id, in_goal);
	}
}

void cDeepMimicCore::AddForce(int agent_id, int part_idx, double force, const std::vector<double>& direction, double duration)
{
	const auto& rl_scene = GetRLScene();
	if (rl_scene != nullptr)
	{
		Eigen::Vector4d in_direction;
		in_direction << direction[0], direction[1], direction[2], 0.0;
		rl_scene->AddForce(agent_id, part_idx, force, in_direction, duration);
	}
}
