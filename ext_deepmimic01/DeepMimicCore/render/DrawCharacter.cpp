...
M4 convert(tMatrix mat)
{
	M4 mat1, mat2;
	float matarray[16];
	for (int i = 0; i < 16; i++)
		matarray[i] = mat(i);
	memcpy(&mat1.m[0][0],matarray, 4*16);
	M4::gl_M4_Transpose(&mat2, &mat1);
	return mat2;
}

void transmat(MimicMotionInfo mminfo)
{
	mminfo.init();
}
...
void cDrawCharacter::DrawCharShapes(const cCharacter& character, const tVector& fill_tint, const tVector& line_col)
{
	...
	for (int i = 0; i < num_shapes; ++i)
	{
		...
		if (i <= 16 && gFrameCount < 4 )
		{
			//std::cout << "-----------------Shape ID -------------------" << std::endl;
			tMatrix local = character.GetJointLocalTrans(i);
			...
			//----------------------------------------------------------------------------
			MatVector[i] = mm;
			V4 quant = mRot2Quat(mm);
			printf("%i, %s, %6.2f, %6.2f, %6.2f, %6.2f\n", i, strnames[i].c_str(), quant.v[0], quant.v[1], quant.v[2], quant.v[3]);
		}
	...
	MimicMotionInfo MMcInfo;
	MMcInfo.init();
	MMcInfo.JointTranformBack(MatVector);
	for (int i = 0; i < 17; i++)
	{
		RotSeq rotSeq;
		rotSeq = zyx;
